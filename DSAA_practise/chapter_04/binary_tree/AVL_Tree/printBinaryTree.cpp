...
int getBinaryTreeHeight(searchTreePtr tree)
{
    if(tree == nullptr)
	return -1;
    else
    {
	int leftHeight = getBinaryTreeHeight(tree->left);
	int rightHeight = getBinaryTreeHeight(tree->right);
...
{
    int treeHeight = getBinaryTreeHeight(tree);
    if(treeHeight == 0)
    {
